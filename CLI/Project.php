...
//            \echoLine("$projectName is now your default project.");
    ...
    protected function _removeproject($projectName) {
        ...
        if (!$simulate) {
            $parameters->deleteProject($projectName);
        }
    }
    
    protected function _cleanProjectList($projectName){
        if (!file_exists($projectDir)) {
            $command = $parameters->getCommandLine();
            if ($command == 'purge') {
                $parameters->deleteProject($projectName);
                //die('ERROR purge');
                $this->_purge($projectName);
                $finalMessage = 'ERR_CANNOTFIND';
            }
            \Messages::Abort("ERR_NEEDPURGE", $projectDir, $projectName);
        }
    ...
    /**
     * 
     * @param string $projectName the project name (in dir1_dir2_file form)
     */
    protected function _purge($projectName) {
        $parameters = Parameters::GetInstance();
        $projects = $parameters->getProjects();
        unset($projects[$projectName]);
        $parameters->saveProject(\TRUE);
    ...
    protected function _preparecode() {
        ...
        foreach ($files as $file) {
            ...
            $source = Code::$ApplicationSource . '/' . basename($file);
