...
/**
 * Browser is a slightly modified class from Chris Schuld
 * see original copyright below.
 * Modified lines are explicity marked
 * Namespace has also been renamed
 *
 * Project IRIS-PHP
 *
 ...
 */
    ...
    // Addition for Iris-PHP made by Jacques Thoorens
    const BROWSER_MIDORI = 'Midori'; // http://midori-browser.org/
...
// addition starting her                
            $this->checkBrowserMidori() || // Addition for Iris-PHP made by Jacques Thoorens   
// end of addition
...
// change starting here                
            $this->checkBrowserKonqueror() ||  // Moved here for Iris-PHP made by Jacques Thoorens  (to prevent Safari detection)
// end of change
...
// change starting here                
            //$this->checkBrowserKonqueror() ||   // Moved up for Iris-PHP made by Jacques Thoorens  (to prevent Safari detection)
// end of change                
...
// Addition for Iris-PHP made by Jacques Thoorens [0] -> [1] to get Safari/AppleWebkit version number instead of Konqueror's
            $aversion = explode('/', $aresult[1]); 
// End of change            
            if (isset($aversion[1])) {
                $this->setVersion($aversion[1]);
...
// Addtion starting here    
    /**
     * Determine if the browser is Midori
     * Addition for Iris-PHP made by Jacques Thoorens
     * @return boolean True if the browser is Vivaldi otherwise false
     */
    protected function checkBrowserMidori()
    {
        if (stripos($this->_agent, 'Midori') !== false) {
            $aresult = explode('/', stristr($this->_agent, 'Midori'));
            if (isset($aresult[1])) {
                $aversion = explode(' ', $aresult[1]);
                $this->setVersion($aversion[0]);
                $this->setBrowser(self::BROWSER_MIDORI);
                return true;
            }
        }
        return false;
