 ...
 */
class SmartTabDiv extends \Iris\views\helpers\_ViewHelper {
     ...
     * @param string $divClass An optional class for the rows
     * @return string the html string corresponding to the inner part of the tab
     */
    public function render($data = [], $col = 3, $divClass = '') {
        if (count($data) == 0) {
            $data = $this->_data;
        } 
        $offset = 0;
        ...
        foreach ($data as $item) {
            $divAttribute = $divClass == '' ? '' : "class=\"$divClass\"";
            $html .= TAB . TAB . "<div $divAttribute>" . CRLF;
            switch ($this->_type) {
                case self::HELPER:
