...
class Date extends TimeDate {
    ...
    public function getHours($format = 'G', $safe = \TRUE) {
        return $this->_invalidTimeFunction(__FUNCTION__, $safe);
    }
    ...
    public function addHour($hour, $clone = \FALSE, $safe = \TRUE) {
        return $this->_invalidTimeFunction(__FUNCTION__, $safe);
    }
     ...
     */
    private function _invalidTimeFunction($type, $safe) {
        $this->isValid(\FALSE);
        if ($safe) {
            throw new \Iris\Exceptions\TimeDateException("Time operation ($type) is not legal on date function");
        }
        return 0;
    }
